...
def viz_results_seq(directory, default_layout=False, colored=False, coords=False, imgW=5000, imgH=5000):
    for filePath in glob.glob(os.path.join(directory, '*.gml')):
        ...
        default_layout = viz_result(filePath, imgFilePath, default_layout=default_layout,
                                    colored=colored, coords=coords, imgW=imgW, imgH=imgH)

def viz_results(directory, default_layout=False, colored=False, coords=False, imgW=5000, imgH=5000,
                vLabel=False, vSize=2, eWidth=10, eWidthMulti=False):
    for filePath in glob.glob(os.path.join(directory, '*.gml')):
        file = os.path.basename(filePath)
        fileName = os.path.splitext(file)[0]
        fileExtension = os.path.splitext(file)[1]
        ...
        default_layout = viz_result(filePath, imgFilePath, default_layout=default_layout, colored=colored,
                                    coords=coords, imgW=imgW, imgH=imgH, vLabel=vLabel, vSize=vSize,
                                    eWidth=eWidth, eWidthMulti=eWidthMulti)

def viz_result(gmlFile, imgFile, imgLayout="fr", imgFormat="png", default_layout=False,
               colored=False, coords=False, imgW=5000, imgH=5000, vLabel=False,
               vSize=10, eWidth=2, eWidthMulti=False, vColor="#0000ff", eColor="#ff0000"):
    elapsedTime = time()
...
#        visual_style["vertex_color"] = ["#000000"] * g.vcount()
        visual_style["vertex_color"] = [vColor] * g.vcount()
    print "\t", timeToStr(time() - elapsedTime)
    elapsedTime = time()
    ...
    if colored == True:        
        ...
        visual_style["edge_color"] = [eColor] * g.ecount()
    print "\t", timeToStr(time() - elapsedTime)
    elapsedTime = time()
...
#            default_layout = [[g.vs["lon"][index], g.vs["lat"][index]] for index in range(len(g.vs["lat"]))]                    
    ...
    print "saving image: ", imgFile , ".", imgFormat, "..."
    plot(g, imgFile + "." + imgFormat, **visual_style)
    print timeToStr(time() - elapsedTime)
    elapsedTime = time()
